...
router.post('/request_applysettle', isLoggedIn, async (req, res) => {
    ...
    for ( let i in list )
    {
        ...
        if ( user != null )
        {
            let iSettleAcc = (user.iSettleAcc); // 전웡이월
            let iSettle = (list[i].iSettle); // 분기 죽장
            let iSettleGive = (list[i].iInputSettle);   // 지급죽장
            let iSettleAccTotal = (user.iSettleAcc); // 총이월(전월죽장이월 + 죽장 - 죽장지급)
            ...
            if ( settle == null )
            {
                ...
                let iSettleUser = (user.iSettle); // 이용자가 가지고 있는 죽장
...
router.post('/request_applysettle_all', isLoggedIn, async (req, res) => {
    ...
    for ( let i in list )
    {
        ...
        if ( user != null )
        {
            ...
            let iSettle = (list[i].iSettle); // 분기 죽장
            let iSettleGive = (list[i].iInputSettle); // 지급죽장
            let iSettleBeforeAcc = (list[i].iSettleBeforeAcc); // 전월죽장이월
            let iSettleAccTotal = (user.iSettleAcc); // 총이월(전월죽장이월 + 죽장이월)
            let iPayback = 0; // 수금
...
let GetSettleVice = (obj) => {
    // 죽장 계산
    let iBTotal = (obj.iBaccaratTotalVice ?? 0) + (obj.iUnderOverTotalVice ?? 0);
    let iSTotal = (obj.iSlotTotalVice ?? 0);
    let iPBATotal = (obj.iPBATotalVice ?? 0);
    let iPBBTotal = (obj.iPBBTotalVice ?? 0);
